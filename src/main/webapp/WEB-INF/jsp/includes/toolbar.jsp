<div class="row">
	<div class="col-sm-8 col-sm-offset-4">
		<div class="col-sm-3">
			<button class="btn btn-default" type="button">Remove
				Order(s)</button>
		</div>
		<div class="btn-group col-sm-5" role="group" aria-label="...">
			<button type="button" class="btn btn-default">Cancel Order</button>
			<button type="button" class="btn btn-default">Send Order</button>

<div class="row toolbar">
	<div class="col-sm-2">
		<div class="btn-group btn-group-justified" role="group"
			aria-label="...">
			<div class="btn-group" role="group">
				<button type="button" class="btn btn-default">Remove Order(s)</button>
			</div>
		</div>
	</div>
	<div class="col-sm-3 col-sm-offset-7">
		<div class="btn-group btn-group-justified" role="group"
			aria-label="...">
			<div class="btn-group" role="group">
				<button type="button" class="btn btn-default">Cancel Block</button>
			</div>
			<div class="btn-group" role="group">
				<button type="button" class="btn btn-default">Send Block</button>
			</div>

		</div>
	</div>
</div>
